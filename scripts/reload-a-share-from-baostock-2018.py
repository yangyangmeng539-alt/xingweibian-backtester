...
def fetch_symbol_rows_once(item):
...
def fetch_symbol_rows_with_retry(item):
    last_error = None

    for attempt in range(1, MAX_RETRY_PER_SYMBOL + 1):
        try:
            return fetch_symbol_rows_once(item)

        except Exception as e:
            last_error = e
            print(f"  retry {attempt}/{MAX_RETRY_PER_SYMBOL} {item['symbol']} {item.get('name') or ''}: {e}")

            # 很多 10054 是连接被服务端断开，重登即可继续。
            try:
                login_baostock()
            except Exception as login_error:
                print("  relogin failed:", login_error)
...
def main():
    ...
    try:
        ...
        print("A股 BaoStock universe:", len(universe))
        print("进度文件:", PROGRESS_PATH)
        ...
        for index, item in enumerate(universe, start=1):
            ...
            if processed_this_run > 1 and processed_this_run % RELOGIN_EVERY == 0:
                print(f"\n=== 定期重登 BaoStock：本轮已处理 {processed_this_run} 只 ===")
                login_baostock()
            ...
            try:
                rows = fetch_symbol_rows_with_retry(item)
                ...
                if not rows:
                    ...
                    print(f"[{index}/{total}] SKIP {symbol} {name} NO_ROWS")
                    continue
                ...
                print(
                    f"[{index}/{total}] DONE {symbol} {name} "
                ...
                print(f"[{index}/{total}] FAIL {symbol} {name} {e}")
                time.sleep(1.2)
